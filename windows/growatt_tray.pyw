...
soc = 0
bat_state = "standby"  # "charge", "discharge" oder "standby"
...
def get_growatt_data():
    ...
    global soc, bat_state, tooltip_text
    try:
        ...
        # Akku-Status & Pfeil-Zustand bestimmen
        if charge_kw > 0.05:
            bat_state = "charge"
        ...
        # Mehrzeiligen Tooltip zusammenbauen (Mouseover-Text)
        lines = [
...
def create_battery_icon(percentage, status="standby"):
    """
    Erstellt  Akku-Icon mit dicken weißen '<' und '>' Symbolen.
    status: "charge", "discharge" oder "standby"
    """
    width = 64
    height = 64
    ...
    # Farbwechsel für den Akku-Füllstand
    if percentage > 30:
        color = (0, 220, 0)      # Grün
    elif percentage > 15:
    ...
    # --- 1. Maximierter Akku ---
    # Gehäuse (Außenrahmen)
    draw.rectangle([2, 8, 54, 56], outline=(255, 255, 255), width=4)
    # Pluspol rechts am Akku
    draw.rectangle([54, 22, 60, 42], fill=(255, 255, 255))

    # Füllstand zeichnen (Innenbereich)
    fill_width = int((44 * percentage) / 100)
    if fill_width > 0:
        draw.rectangle([6, 12, 6 + fill_width, 52], fill=color)

    # --- 2. Dickes weißes Symbol im Akku mit schwarzer Kontur ---
    symbol_color = (255, 255, 255)  # Weiß
    outline_color = (0, 0, 0)        # Schwarzer Rand für besseren Kontrast

    if status == "charge":
        # Dickes '>' Symbol (Laden)
        # Schwarze Kontur zuerst zeichnen
        draw.line([(22, 18), (36, 32)], fill=outline_color, width=9)
        draw.line([(36, 32), (22, 46)], fill=outline_color, width=9)
        # Weißer Kern darüber
        draw.line([(22, 18), (36, 32)], fill=symbol_color, width=5)
        draw.line([(36, 32), (22, 46)], fill=symbol_color, width=5)

    elif status == "discharge":
        # Dickes '<' Symbol (Entladen)
        # Schwarze Kontur zuerst zeichnen
        draw.line([(36, 18), (22, 32)], fill=outline_color, width=9)
        draw.line([(22, 32), (36, 46)], fill=outline_color, width=9)
        # Weißer Kern darüber
        draw.line([(36, 18), (22, 32)], fill=symbol_color, width=5)
        draw.line([(22, 32), (36, 46)], fill=symbol_color, width=5)

    return image


def update_loop(icon):
    """Hintergrund-Thread für den periodischen Datenabruf"""
    while is_running:
        get_growatt_data()

        # Icon-Grafik und Tooltip-Text aktualisieren
        icon.icon = create_battery_icon(soc, bat_state)
...
initial_icon = create_battery_icon(0, "standby")
